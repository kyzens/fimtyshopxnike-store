...
const CartItem = ({ item: { id, title, text, img, color, shadow, price, cartQuantity } }) => {
  ...
  const resultPxQTY = price * cartQuantity;

  const priceFormat = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(price);
  const priceXQtyFormat = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(resultPxQTY);
      ...
      <div className="flex items-center justify-between w-full px-5">
        <div className="flex items-center gap-5">
          <div className={`bg-gradient-to-b ${color} ${shadow} relative rounded p-3 hover:scale-105 transition-all duration-75 ease-in-out grid items-center`}>
            ...
            <div className='absolute right-1 top-1 blur-theme-effect bg-white/80 text-black text-xs px-1 rounded'>{priceFormat}</div>
          </div>
        ...
        <div className="grid items-center gap-5">
          <div className="grid items-center justify-center">
            <h1 className="text-md lg:text-base text-slate-900 font-medium">{priceXQtyFormat}</h1>
          </div>
          <div className="grid items-center justify-center">
